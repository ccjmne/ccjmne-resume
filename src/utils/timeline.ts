...
export function render(timeline: string[], range: number[]): EasyHTMLElement[] {
  ...
  const branches = compute(timeline.toReversed())
  ...
  const graph = elementSVG('g').attrs({ mask: `url(#timeline-mask)` }).content(...branches.flatMap(function({ depth, events }: Branch, i): EasyHTMLElement[] {

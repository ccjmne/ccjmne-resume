...
milestones.map(c => (parser.exec(c) as RegExpWGroups<'pipes' | 'what'>).groups)
...
document.fonts.ready.then(function() {
  ...
  const pivots = milestones.map((c, i) => [c, i] as const).filter(([c]) => c.includes('★')).map(([, i]) => i)
  // TODO: rename `asdf` and `asdf2`
  const asdf = zip([0, ...pivots, milestones.length], [height, ...highlights.toReversed().map(({ top, height }) => top + height / 2), 0])
